...
export const useGetContractExtensions = (contract_id: string) => {
    const getData = () => contractController.getContractExtensions(contract_id);

    return useQuery({
        queryKey: ['ContractsExtensions'],
        queryFn: () => getData(),
        staleTime: 0,
        refetchOnWindowFocus: false,
        refetchOnMount: false,
